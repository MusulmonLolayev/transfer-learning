...
from utils import crit_dividing, class_direction, text_model_links, text_prep_links
...
def experiment(
  ...
  try:
    base_model = hub.KerasLayer(model_link, trainable=False, name='bert_encoder')
    prep_layer = hub.KerasLayer(prep_link, name='preprocessing')
  except:
    print('The pre-trained model not found from the kaggle server')
    print('The pre-trained model not found from the kaggle server', file=f)
  ...
  # build model again
  net = layers.Dense(num_classes, name='classifier')(net)
  model = tf.keras.Model(text_input, net)
...
def main():
  ...
  parser.add_argument('--model', type=str, choices=list(text_model_links.keys()))
  ...
  with open(log_f_path, 'w') as f:
    ...
    # # to test code
    # version = "small_bert/bert_en_uncased_L-4_H-512_A-8"
    # link = text_model_links['small_bert'][version]
    # prep_link = text_prep_links[version]
    ...
    print("="*30, "Running all text classification problems", "="*30)
    print("="*30, "Running all text classification problems", "="*30, file=f)
    print('\n', "*"*30, f"Model name: {model_name}", "*"*30)
    print('\n', "*"*30, f"Model name: {model_name}", "*"*30, file=f)
    for version, link in text_model_links[model_name].items():
      prep_link = text_prep_links[version]
